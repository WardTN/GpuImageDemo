...
import com.example.gpuimagedemo.util.TextureRotationUtil;
...
import static com.example.gpuimagedemo.util.TextureRotationUtil.TEXTURE_NO_ROTATION;
...
public class GPUImageRenderer implements GLSurfaceView.Renderer, GLTextureView.Renderer, Camera.PreviewCallback {
    ...
    private static final int NO_IMAGE = -1; //没有照片
    ...
    private GPUImageFilter filter;  //滤镜基类
    ...
    private int outputWidth;    //画布宽
    private int outputHeight;   //画布长
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        GLES20.glClearColor(backgroundRed, backgroundGreen, backgroundBlue, 1); //设置背景
        GLES20.glDisable(GLES20.GL_DEPTH_TEST);
        filter.ifNeedInit();    //滤镜初始化
    }
    ...
    public void onSurfaceChanged(final GL10 gl, final int width, final int height) {
        ...
        filter.onOutputSizeChanged(width, height);  //设置滤镜大小
        adjustImageScaling();   //调整图片大小

        synchronized (surfaceChangedWaiter) {
            surfaceChangedWaiter.notifyAll(); //唤醒对象的等等待池中的所有线程，进入锁池
            //notify()方法随机唤醒对象的等待池 中的一个线程
        }
    }
    ...
    public void onPreviewFrame(byte[] bytes, Camera camera) {
        ...
        onPreviewFrame(bytes, previewSize.width, previewSize.height);
    }
